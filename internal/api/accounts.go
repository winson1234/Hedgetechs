...
func CreateAccount(w http.ResponseWriter, r *http.Request) {
	...
	// Check if user already has an account of this type (restriction: 1 live + 1 demo max)
	var existingAccountTypeCount int
	err = tx.QueryRow(ctx, 
		"SELECT COUNT(*) FROM accounts WHERE user_id = $1 AND account_type = $2::account_type_enum", 
		userID, req.Type,
	).Scan(&existingAccountTypeCount)
	if err != nil {
		log.Printf("CreateAccount: Failed to check existing account type - Error: %v, UserID: %v, Type: %v", err, userID, req.Type)
		respondWithJSONError(w, http.StatusInternalServerError, "database_error", fmt.Sprintf("failed to check existing accounts: %v", err))
		return
	}

	if existingAccountTypeCount > 0 {
		accountTypeName := "live"
		if req.Type == models.AccountTypeDemo {
			accountTypeName = "demo"
		}
		log.Printf("CreateAccount: User %v already has a %s account", userID, accountTypeName)
		respondWithJSONError(w, http.StatusBadRequest, "account_limit_reached", 
			fmt.Sprintf("You already have a %s account. Each user can only have one live account and one demo account.", accountTypeName))
		return
	}
	...
	// Insert account into database (account_id auto-generated by BIGSERIAL)
	// Cast string values to enum types for PostgreSQL
	_, err = tx.Exec(ctx,
		`INSERT INTO accounts (id, user_id, account_type, currency, balance, status, last_updated, created_at)
		 VALUES ($1, $2, $3::account_type_enum, $4, $5, $6::account_status_enum, NOW(), NOW())`,
		account.ID, account.UserID, account.Type, account.Currency, initialBalance, accountStatus,
	)
	if err != nil {
