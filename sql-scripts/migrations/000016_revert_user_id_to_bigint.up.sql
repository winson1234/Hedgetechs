-- Migration 000016: Revert accounts.user_id from UUID back to bigint
-- This restores backward compatibility with the middleware that passes bigint user_id
...
ALTER TABLE accounts ADD COLUMN user_id_bigint BIGINT;

-- Populate user_id_bigint from users table (lookup UUID -> bigint user_id)
UPDATE accounts a
SET user_id_bigint = u.user_id
FROM users u
...
-- Rename user_id_bigint to user_id
ALTER TABLE accounts RENAME COLUMN user_id_bigint TO user_id;

-- Set NOT NULL constraint
ALTER TABLE accounts ALTER COLUMN user_id SET NOT NULL;

-- Add foreign key constraint referencing users.user_id (bigint)
...
COMMENT ON COLUMN accounts.user_id IS 'Bigint user_id from users table (for middleware compatibility)';
